<template>
  <div>
    ...
    <v-row>
       <v-col lg="12">
         <v-data-table  caption="" :headers="headers" :items="clients" :items-per-page="5" class="elevation-2">
          
           <template v-slot:item.actions="{ item }">
             <v-row>
      <DetailsClient class="mr-1"/>
      <EditClient class="mr-1"
       :item="item" />
...
export default {
  ...
  methods:{
   deleteItem(item){
      ...
      axios.delete('http://localhost:3000/client/'+item.id)
      .then(response=>{
